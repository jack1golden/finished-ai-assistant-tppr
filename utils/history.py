...
def init_if_needed(DETECTORS: Dict[str, list], days: int = 180, step_minutes: int = 15, spikes_per_week: int = 1):
    """Seed 6 months by default, with 1 spike/week per detector, 15-min resolution."""
    ...
    time_index = pd.date_range(start, now, freq=f"{step_minutes}min")
...
def apply_runtime_ops(df: pd.DataFrame, room: str, label: str, simulate: bool, ops: dict) -> pd.DataFrame:
    """Overlay runtime demo effects on top of history (for last ~20 samples)."""
    if df is None or df.empty:
        return df
    out = df.copy()
